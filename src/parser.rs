...
fn parse_expr(tokens: &mut tokenstream!(), min_bp: u8) -> Result<Expr, ParseError> {
    ...
    loop {
        expect_token_peek!(tokens, tok, match tok.inner() {
            ...
            // INFIX OPERATORS
            Token::Star if min_bp < /* left-bp */ 13 => {tokens.next(); lhs = expr_binary!(Token::Star, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 14)); continue},
            Token::Div if min_bp < /* left-bp */ 13 => {tokens.next(); lhs = expr_binary!(Token::Div, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 14)); continue},
            Token::Plus if min_bp < /* left-bp */ 9 => {tokens.next(); lhs = expr_binary!(Token::Plus, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 10)); continue},
            Token::Minus if min_bp < /* left-bp */ 9 => {tokens.next(); lhs = expr_binary!(Token::Minus, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 10)); continue},
            ...
            Token::And if min_bp < /* left-bp */ 5 => {tokens.next(); lhs = expr_binary!(Token::And, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 6)); continue},
            Token::Or if min_bp < /* left-bp */ 3 => {tokens.next(); lhs = expr_binary!(Token::Or, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 4)); continue},
            Token::Equals if min_bp < /* left-bp */ 7 => {tokens.next(); lhs = expr_binary!(Token::Equals, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 8)); continue},
            Token::NotEquals if min_bp < /* left-bp */ 7 => {tokens.next(); lhs = expr_binary!(Token::NotEquals, expect_box!(lhs), expr_boxed!(tokens, /* right-bp */ 8)); continue},
            Token::Star => break,
            Token::Div => break,
            Token::Plus => break,
            Token::Minus => break,
